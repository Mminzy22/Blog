...
### **목차**
1. [지도 학습의 이해: 분류와 회귀](#지도-학습의-이해-분류와-회귀)  
   - [지도 학습이란?](#지도-학습이란)  
   - [분류(Classification)](#분류classification)  
   - [회귀(Regression)](#회귀regression)  
   - [분류와 회귀의 차이점](#분류와-회귀의-차이점)  
   - [분류와 회귀를 혼동하기 쉬운 사례](#분류와-회귀를-혼동하기-쉬운-사례)  
2. [k-최근접 이웃 알고리즘](#k-최근접-이웃-알고리즘)  
   - [k-최근접 이웃 분류](#k-최근접-이웃-분류)  
   - [k-최근접 이웃 회귀](#k-최근접-이웃-회귀)  
3. [데이터 준비 과정](#데이터-준비-과정)  
   - [넘파이 배열 생성](#넘파이-배열-생성)  
   - [산점도를 이용한 데이터 시각화](#산점도를-이용한-데이터-시각화)  
   - [데이터셋 분리: 훈련 세트와 테스트 세트](#데이터셋-분리-훈련-세트와-테스트-세트)  
4. [데이터의 차원 조정](#데이터의-차원-조정)  
   - [넘파이 `reshape()` 메서드의 이해와 활용](#넘파이-reshape-메서드의-이해와-활용)  
   - [데이터셋을 2차원 배열로 변환](#데이터셋을-2차원-배열로-변환)  
5. [k-최근접 이웃 회귀 모델 훈련](#k-최근접-이웃-회귀-모델-훈련)  
   - [KNeighborsRegressor로 회귀 모델 생성 및 훈련](#kneighborsregressor로-회귀-모델-생성-및-훈련)  
   - [결정계수 \(R^2\): 모델 평가 지표](#결정계수-r2-모델-평가-지표)  
6. [평균 절댓값 오차 계산](#평균-절댓값-오차-계산)  
   - [`mean_absolute_error()`로 예측 오차 측정](#mean_absolute_error로-예측-오차-측정)  
   - [실제 예측과 타깃값의 차이 분석](#실제-예측과-타깃값의-차이-분석)  
7. [모델 평가 및 과대적합과 과소적합](#모델-평가-및-과대적합과-과소적합)  
   - [훈련 세트와 테스트 세트의 평가](#훈련-세트와-테스트-세트의-평가)  
   - [과대적합과 과소적합의 정의와 해결 방법](#과대적합과-과소적합의-정의와-해결-방법)  
8. [k-최근접 이웃 모델의 복잡도 조정](#k-최근접-이웃-모델의-복잡도-조정)  
   - [이웃의 개수 (\(k\)) 조정으로 모델 성능 개선](#이웃의-개수-k-조정으로-모델-성능-개선)  
   - [\(k\) 값 변화에 따른 과대적합과 과소적합 해결 사례](#k-값-변화에-따른-과대적합과-과소적합-해결-사례)  
9. [핵심 요약](#핵심-요약)  
   - [회귀 문제의 정의와 k-최근접 이웃 회귀의 특징](#회귀-문제의-정의와-k-최근접-이웃-회귀의-특징)  
   - [결정계수 (\(R^2\))의 의미와 중요성](#결정계수-r2의-의미와-중요성)  
   - [과대적합과 과소적합의 구분 및 해결 방안](#과대적합과-과소적합의-구분-및-해결-방안)  
10. [필수 패키지와 함수 요약](#필수-패키지와-함수-요약)  
    - [scikit-learn 주요 클래스와 함수](#scikit-learn-주요-클래스와-함수)  
    - [numpy의 `reshape()` 메서드와 활용법](#numpy의-reshape-메서드와-활용법)  
    - [matplotlib를 활용한 데이터 시각화](#matplotlib를-활용한-데이터-시각화)  
